...
void NPlayer::readMessage(const QString &str)
{
    ...
    if (!files.isEmpty()) {
        if (NSettings::instance()->value("EnqueueFiles").toBool()) {
            ssize_t lastRow = m_playlistController->model()->size() - 1;
...
void NPlayer::loadDefaultPlaylist()
{
    ...
    if (!playlistRowValues.isEmpty()) {
        ...
        if (!m_settings->value("StartPaused").toBool()) {
            ...
            QString file = model->data(row, NPlaylistModel::FilePathRole).toString();
            int id = model->data(row, NPlaylistModel::IdRole).toInt();
...
{
    int row = m_playlistController->model()->playingRow();
...
{
    m_preferencesDialogHandler->showDialog();
}
