...
NShortcutEditorModel::NShortcutEditorModel(NPlayer *player) : QAbstractListModel(player)
{
    for (NAction *action : player->findChildren<NAction *>()) {
        if (action->objectName().isEmpty() || !action->isCustomizable()) {
            continue;
        }
        m_actionList << action;
        m_shortcuts << action->shortcuts().join(", ");
        m_globalShortcuts << action->globalShortcuts().join(", ");
    }
}
...
void NShortcutEditorModel::applyShortcuts()
{
    for (int i = 0; i < m_actionList.size(); ++i) {
        ...
        action->setShortcuts(m_shortcuts.at(i).split(", "));
        action->setGlobalShortcuts(m_globalShortcuts.at(i).split(", "));
    }
}
