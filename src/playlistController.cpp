...
void NPlaylistController::keyPress(int key, Qt::KeyboardModifiers modifiers)
{
    ...
    if (!modifiers) {
        ...
        if (oldFocusedRow < focusedRow) {
            m_model->setData(oldFocusedRow, focusedRow, NPlaylistModel::IsSelectedRole, true);
        } else {
            m_model->setData(focusedRow, oldFocusedRow, NPlaylistModel::IsSelectedRole, true);
        }
        return;
    } else if (modifiers & Qt::ControlModifier) {
        switch (key) {
