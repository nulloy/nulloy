...
int NPlaylistController::rowsPerPage() const
{
    return m_rowsPerPage;
}

int NPlaylistController::firstVisibleRow() const
{
...
bool NPlaylistController::repeatMode() const
{
    return m_settings.value("Repeat").toBool();
}

void NPlaylistController::setRepeatMode(bool repeat)
{
...
void NPlaylistController::removeSelected()
{
    ...
    if (selectedRows.indexOf(m_model->playingRow()) != -1) {
    ...
    if (rowToFocus >= newSize) { // last row was within removed
        if (playingItemRemoved && m_settings.value("LoopPlaylist").toBool()) {
            rowToFocus = 0;
        } else {
            rowToFocus = newSize - 1;
        }
    }
...
void NPlaylistController::playRow(int row)
{
    if (row == -1) {
        ...
        m_model->setData(-1, NPlaylistModel::IsPlayingtRole, false);
        return;
    }
