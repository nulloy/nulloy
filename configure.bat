...
set EMBED_GSTREAMER=no
set BUILD_TAGLIB=no
set BUILD_VLC=no
set BUILD_PHONON=no
set SUPPORT_SKINS=yes
set SUPPORT_PLUGINS=yes
...
:getopt
	...
	if "%0" == "--embed-gstreamer" (
		set EMBED_GSTREAMER=yes
		goto getopt
	)
	if "%0" == "--vlc" (
		set BUILD_VLC=yes
...
if "%EMBED_GSTREAMER%" == "yes"           echo CONFIG += embed-gstreamer>> %QMAKE_CACHE%
...
if "%SUPPORT_PLUGINS%" == "no"            echo CONFIG += no-plugins>> %QMAKE_CACHE%
